...
@click.command
@click.option(
    "--model", default=ModelType.XGBOOST, type=click.Choice(ModelType.ALL, case_sensitive=False)
)
@click.option(
    "--target", default=TargetType.SBP, type=click.Choice(TargetType.ALL, case_sensitive=False)
)
@click.option(
    "--panel", default=PanelType.WHOLE, type=click.Choice(PanelType.ALL, case_sensitive=False)
)
@click.option(
    "--threshold",
    type=float,
...
@click.option("--n_trials", type=int, default=5, metavar="N", help="number of trials for Optuna")
@click.option(
    "--no_opt",
    is_flag=True,
    default=False,
