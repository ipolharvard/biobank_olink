...
def cross_sectional(
    ...
    model = Model(model)
    panel = Panel(panel)
    study_name = get_study_name(exp_name="cross_sectional", model=model, panel=panel, nan_th=nan_th,
                                corr_th=corr_th)

    ol_df, cov_df = load_olink_and_covariates(nan_th, nan_th, corr_th)
    # take those patients who had no HTN at the baseline and has never had I10 before
    cov_df = cov_df.loc[(cov_df.HTNgroup <= 1) & (cov_df.timeHTNcoxfinal >= 0)].copy()
    y = cov_df.HTNgroup.values
