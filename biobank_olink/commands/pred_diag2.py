...
from biobank_olink.constants import Model, Panel, OPTUNA_DB_DIR, RESULTS_DIR
...
def pred_diag2(
    ...
    model = Model(model)
    panel = Panel(panel)
    study_name = get_study_name(exp_name="pred_diag2", model=model, panel=panel, nan_th=nan_th,
                                corr_th=corr_th)
    ...
    for i, seed in enumerate(range(0, 100), 1):
        ...
        estimator = XGBClassifier(tree_method="hist", device="cuda", random_state=42, **best_params)
        ...
        logger.info("Coming up with random group assignment...")
